...
const EfsDirectorySuffix = "_efs"
const EfsMountTimeout = time.Minute * 5
...
func NewEfsMounter(
	...
	return &efsMounter{invoker, osshim, syscallshim, ioutilshim, mountChecker, fstype, defaultOpts, resolver, mask, efsPath}
}

func (m *efsMounter) Mount(env dockerdriver.Env, remote string, target string, opts map[string]interface{}) error {
	...
	intermediateMount := target + EfsDirectorySuffix
	...
	if uidok {
		// make sure the mapped user has read access to the directory before doing the efs mount
		...
		args := efsOptions(optsToUse)
		args = append(args, target, intermediateMount)
		mountError := m.invoker.Invoke(env, m.efsPath, args).WaitFor("Mounted!", EfsMountTimeout)
		if mountError != nil {
...
func (m *efsMounter) Purge(env dockerdriver.Env, path string) {
	...
	pkillInvokeResult := m.invoker.Invoke(env, "pkill", []string{"efs"})
	...
	for i := 0; i < 30; i++ {
		...
		invokeResult := m.invoker.Invoke(env, "pgrep", []string{"efs"})
	...
	mountPattern, err := regexp.Compile("^" + path + ".*" + EfsDirectorySuffix + "$")
	if err != nil {
	...
	for _, mountDir := range mounts {
		realMountpoint := strings.TrimSuffix(mountDir, EfsDirectorySuffix)
		...
		if err != nil {
			logger.Error("warning-umount-efs-failed", err)
		}
...
func efsOptions(opts vmo.MountOpts) []string {
	var ret []string
